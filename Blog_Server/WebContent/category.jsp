...
<%
	//判断有没有用户数据，如果没有则跳转到category.s?op=query
	if(request.getAttribute("categroyList") == null){
		request.getRequestDispatcher("category.s?op=query").forward(request, response);
	}
%>	
<!doctype html>
<html lang="zh-CN">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
        <div class="col-md-5">
          ...
          <form action="category.s" method="post" autocomplete="off">
          <input type="hidden" name="op" value="add">
            <div class="form-group">
              <label for="category-name">栏目名称</label>
              <input type="text" id="category-name" name="columnName" class="form-control" 
              	placeholder="在此处输入栏目名称" value="${param.columnName}" autocomplete="off">
              <span class="prompt-text">这将是它在站点上显示的名字。</span> 
            </div>
            <div class="form-group">
              <label for="category-alias">栏目别名</label>
              <input type="text" id="category-alias" name="aliasName" 
              		class="form-control" placeholder="在此处输入栏目别名" value="${param.aliasName}" autocomplete="off">
            ...
            <div class="form-group">
              ...
              value="${param.keyWords}" placeholder="在此处输入栏目关键字" autocomplete="off">
              <span class="prompt-text">关键字会出现在网页的keywords属性中。</span> 
            </div>
            <div class="form-group">
              <label for="category-describe">描述</label>
              <textarea class="form-control" id="category-describe" name="description" rows="4" 
              value="${param.description}" autocomplete="off"></textarea>
        ...
        <div class="col-md-7">
          ...
          <div class="table-responsive">
            <table class="table table-striped table-hover">
              ...
              <tbody>
               <c:forEach items="${categroyList}" var="u">
                <tr>
                  <td>${u.id }</td>
                  <td>${u.columnName }</td>
                  <td>${u.aliasName }</td>
